...
const moderators = ["CatR3kd"];

// Automatic message

const autoMessage = true;
const autoMessageTxt =
  "Спасибо за игру! Обязательно поставьте лайк, если вам понравилось. Люди, которые оставляют отзыв, награждаются эксклюзивной ролью в чате (; Discord - https://discord.gg/ppPZX4D7Wf";
...
io.on("connection", (socket) => {
  ...
  socket.on("signUp", (user) => {
    createAccount(user.username, user.password)
      .then(function (res) {
        if (res.error == false) {
          const userObj = {
  ...
  socket.on("login", (user) => {
    ...
    login(user.username, user.password, socket)
      .then(function (res) {
        if (res.error == false) {
          const userObj = {
            username: res.message.username,
  ...
  socket.on("onlineUsersRequest", async () => {
    try {
      await usersOnlineRateLimit.consume(
  ...
  socket.on("hack", async (hackObj) => {
    ...
    if (!user)
      return socket.emit("hackReturn", {
        error: true,
        message: "user does not exist",
      });

    if (!validateUserWithSocket(hackObj.hacker.username, socket.id))
      return socket.emit("hackReturn", {
        error: true,
    ...
    if (!connectedUsers.has(hackObj.victim))
      return socket.emit("hackReturn", {
        error: true,
    ...
    if (victim.gameStats.money <= 10000)
      return socket.emit("hackReturn", {
        error: true,
        message: "victim does not have enough money",
      });

    // Make sure victim is not being hacked
    if (usersBeingHacked.has(victim.username))
      return socket.emit("hackReturn", {
        error: true,
        message: "victim is already being hacked",
      });

    // Make sure victim is not the hacker
    if (hackObj.hacker.username == hackObj.victim)
      return socket.emit("hackReturn", {
        error: true,
        message: "you cannot hack yourself",
      });
    ...
    try {
      ...
      // Ratelimited
      socket.emit("hackReturn", {
        error: true,
        message: `still on cooldown for ${
          Math.round(rejRes.msBeforeNext / 60000) + 1
        } minute(s)`,
      });
    }
  });
  ...
  socket.on("preventHack", async (code) => {
    if (gameHacks.has(code)) {
      gameHacks.set(code, {
        prevented: true,
        value: gameHacks.get(code).value,
        accountedFor: false,
      });
    }
  });
...
function validateUserWithSocket(username, socketID) {
  if (!(connectedUsers.has(username) && connectedUsers.has(socketID)))
    return false;

  return (
    connectedUsers.get(username) === socketID &&
    connectedUsers.get(socketID).username === username
  );
}
...
async function createAccount(username, password) {
  ...
  if (username.length > 15)
    return { error: true, message: "Username too long!" };

  if (username.length < 3)
    return { error: true, message: "Username too short!" };
  ...
  if (username.split("").includes(" "))
    return { error: true, message: "Username cannot include spaces!" };

  if (password.length > 25)
    return { error: true, message: "Password too long!" };

  if (password.length < 6)
    return { error: true, message: "Password too short!" };

  if (filter.isProfane(username))
    return { error: true, message: "Username cannot contain profanity!" };

  if (!username || !password)
    return { error: true, message: "Username and password cannot be empty!" };
...
async function login(username, password) {
  ...
  if (user.suspentionStatus.suspended === true)
    return { error: true, message: "User is suspended!" };

  // Check if password matches
  const match = await bcrypt.compare(password, user.password);
  if (match === false || !password)
...
function logModeration(item) {
  ...
  fs.writeFileSync(
    "Logs/ModerationLogs.txt",
    `${oldLogs}${getFormattedDate()} ${item}\n`
  );
}
...
async function updateUserData(userObj, socket) {
  ...
  if (
    user &&
    user.password &&
    validateUserWithSocket(userObj.username, socket.id)
  ) {
    ...
    await setUser(
      ...
      user.password,
      user.gameStats,
      user.suspentionStatus
    );

    const validityCheck = checkUpdateValidity(
      userObj.gameStats,
      user.gameStats,
      userObj.username
    );
...
function checkUpdateValidity(newGameStats, oldGameStats, username) {
  ...
  if (oldGameStats.reset.minCost == newGameStats.reset.minCost) {
    autoSpent = calculateSpent(
      newGameStats.auto.value - oldGameStats.auto.value,
      200,
      oldGameStats.auto.value,
      1.145
    );
    mpcSpent = calculateSpent(
      newGameStats.mpc.value - oldGameStats.mpc.value,
      50,
      oldGameStats.mpc.value,
      1.1
    );
    superAutoSpent = calculateSpent(
      newGameStats.superAuto.value - oldGameStats.superAuto.value,
      10000,
      oldGameStats.superAuto.value,
      1.145
    );
    tripleSpent = calculateSpent(
      newGameStats.triple.value - oldGameStats.triple.value,
      500,
      oldGameStats.triple.value,
      1.145
    );
  } else {
  ...
  if (
    newGameStats.otherUpgrades.higherHackAmount == true &&
    oldGameStats.otherUpgrades.higherHackAmount == false
  )
    minimumSpent += 1000000;

  if (
    newGameStats.otherUpgrades.betterFirewall == true &&
    oldGameStats.otherUpgrades.betterFirewall == false
  )
    minimumSpent += 1000000;

  if (
    newGameStats.otherUpgrades.covertHacks == true &&
    oldGameStats.otherUpgrades.covertHacks == false
  )
    minimumSpent += 1000000;

  if (
    newGameStats.otherUpgrades.doubleHacks == true &&
    oldGameStats.otherUpgrades.doubleHacks == false
  )
    minimumSpent += 10000000;
  ...
  const maximumClicked =
    newGameStats.mpc.amount * secondsBetweenUpdate * maximumAvgPerSecond;
  const maximumAuto =
    (newGameStats.auto.amount +
      newGameStats.triple.amount * 3 +
      newGameStats.superAuto.amount * newGameStats.mpc.amount) *
    secondsBetweenUpdate *
    newGameStats.speed.multiplier;
  ...
  const returnObj = {
    ...
    details: {
      margin:
        newGameStats.money -
        (oldGameStats.money + maximumGenerated - minimumSpent),
...
function calculateSpent(
  ...
  basePrice,
  beginningAmount,
  incrementalExponenent
) {
  var total = 0;
  for (let purchases = 0; purchases < amount; purchases++) {
    total += Math.round(
      basePrice * incrementalExponenent ** (beginningAmount + purchases)
    );
  }
  return total;
}
...
async function hackPlayer(hackObj, user, victim, socket) {
  ...
  if (user.gameStats.otherUpgrades.higherHackAmount == true)
    hackValue = Math.round(victim.gameStats.money * 0.15);
  ...
  io.to(connectedUsers.get(victim.username)).emit("hacked", {
    code: code,
    value: gameHacks.get(code).value,
  });
  ...
  if (victim.gameStats.otherUpgrades.betterFirewall == true)
    firewallTime = 25000;
  ...
  setTimeout(function () {
    ...
    if (hack.prevented == true) {
      socket.emit("hackReturn", {
        error: true,
        message: "hack was blocked by victim",
      });
      setTimeout(function () {
      ...
      if (user.gameStats.otherUpgrades.covertHacks == false) {
        const chatObj = {
          sender: "Game",
          message: `${
            hackObj.hacker.username
          } has successfully hacked $${formatNumber(hack.value)} from ${
            hackObj.victim
          }!`,
          badgeColor: "red",
        };
      ...
      if (gameHacks.has(victim.username)) {
        ...
        gameHacks.set(victim.username, {
          prevented: false,
          value: oldVictimHackValue - hack.value,
          accountedFor: false,
        });
      } else {
        gameHacks.set(victim.username, {
      ...
      if (gameHacks.has(user.username)) {
        ...
        gameHacks.set(user.username, {
          prevented: oldHackerHackValue.prevented,
          value: oldHackerHackValue.value + hack.value,
          accountedFor: oldHackerHackValue.accountedFor,
        });
      } else {
...
async function updateTips() {
  await fetch(
    "https://catr3kd-tip-api.catr3kd.repl.co/all?replId=e18389d9-cf4f-4d67-adb2-f0cc759a4097",
    { method: "GET" }
  )
    .then((r) =>
      r.json().then((res) => {
  ...
  await fetch(
    "https://catr3kd-tip-api.catr3kd.repl.co/top?replId=e18389d9-cf4f-4d67-adb2-f0cc759a4097",
    { method: "GET" }
  )
    .then((r) =>
      r.json().then((res) => {
